...
func findLines(re *regexp.Regexp, f string) ([]string, error) { // (2) Change function signature
	file, err := os.Open(f)
	if err != nil {
		return nil, err
	}
	...
	for scanner.Scan() {
		s := scanner.Text()
		if re.MatchString(s) { // (3) Change to use pre compiled regexp
